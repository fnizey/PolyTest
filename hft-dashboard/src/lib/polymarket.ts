...
export async function fetchActiveMarkets(limit = 20): Promise<Market[]> {
  ...
  // Gamma API returns either a plain array or { markets: [...] } or { data: [...] }
  if (Array.isArray(data)) return data
  if (Array.isArray(data.markets)) return data.markets
  if (Array.isArray(data.data)) return data.data
  // Some responses wrap further
  const keys = Object.keys(data)
  for (const k of keys) {
    if (Array.isArray(data[k]) && data[k].length > 0) return data[k]
  }
  console.error('fetchActiveMarkets: unexpected shape', JSON.stringify(data).slice(0, 200))
  return []
}
