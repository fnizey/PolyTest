...
export function MarketCard({ market }: { market: Market }) {
  ...
  const p = market.last_trade_price ?? market.spread?.midprice ?? 0.5
  const midPct = (p * 100).toFixed(1) + 'c'
  const distFrom50 = Math.abs(p - 0.5)
  const distColor = distFrom50 < 0.1
    ? 'text-accent-green'
    : distFrom50 < 0.25
    ? 'text-accent-amber'
    : 'text-zinc-500'
    ...
    <div className="rounded-xl border border-border bg-surface-1 overflow-hidden hover:border-zinc-700 transition-colors">
      ...
      >
        ...
        <div className="text-right shrink-0">
          <div className={clsx('text-lg font-mono font-semibold', distColor)}>
            {midPct}
          </div>
          <div className={clsx('text-xs font-mono', spreadColor)}>
            {spreadBps !== undefined ? spreadBps.toFixed(0) + ' bps' : '--'}
          </div>
          <div className={clsx('text-xs', distFrom50 < 0.1 ? 'text-accent-green' : 'text-zinc-600')}>
            {distFrom50 < 0.05 ? 'near 50' : (distFrom50 * 100).toFixed(0) + 'c from 50'}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 px-4 pb-3">
        ...
        <SignalBadge
          label="OFI"
          value={market.signals.ofi > 0 ? '+' + market.signals.ofi : '' + market.signals.ofi}
          variant={ofiVariant(market.signals.ofi)}
        />
        ...
        {market.signals.as_half_spread !== null && (
          <SignalBadge label="AS d*" value={market.signals.as_half_spread.toFixed(4)} variant="purple" />
        )}
        ...
        {market.one_day_change !== undefined && (
          <SignalBadge
            label="1d chg"
            value={(market.one_day_change > 0 ? '+' : '') + (market.one_day_change * 100).toFixed(1) + 'c'}
            variant={market.one_day_change > 0 ? 'green' : market.one_day_change < 0 ? 'red' : 'neutral'}
          />
        )}
      </div>
